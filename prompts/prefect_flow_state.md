...
# проверь код ниже и сверься с официальным репозиторием prefect на предмет того как правильно выполнять анализ запущенной flow в docker контейнере

сейчас логика работает неверно и я не понимаю статус задачи если она завершена или процент ее выполнения.

[https://github.com/PrefectHQ/prefect](https://github.com/PrefectHQ/prefect)
[https://docs.prefect.io/integrations/prefect-github](https://docs.prefect.io/integrations/prefect-github)
...
Вот полный анализ проблем в коде и как их исправить согласно официальной документации Prefect v3.

## Проблемы в текущем коде

### 1. Неправильное чтение state type

Текущий код в `_poll_run()`:

```python
run["state"] = (info.get("state") or {}).get("type", run["state"])
```

По документации Prefect v3, объект `state` из REST API имеет структуру `{"type": "RUNNING", "name": "Running", ...}`. Тип — `state.type`, имя — `state.name`. Это работает, **но** нужно также учитывать промежуточные типы:[^1]

- `RUNNING` может быть `Running` **или** `Retrying` — оба не терминальные
- `SCHEDULED` может быть `Late` — тоже не терминальный, но требует отображения предупреждения[^1]
- `PAUSED` / `CANCELLING` — вообще отсутствуют в `_TERMINAL_STATES`


### 2. Прогресс-бар не отражает реальность

Задача запускает **один** flow `export-eis-package-deployment`, который внутри вызывает 11 субфлоу. Чтобы получить реальный прогресс — нужно запрашивать **child flow runs** через `/flow_runs/filter` с фильтром по `parent_flow_run_id`. Текущий код этого не делает и просто показывает статичное `0.05`.

### 3. `_TERMINAL_STATES` неполный

Согласно документации, терминальные states: `COMPLETED`, `FAILED`, `CRASHED`, `CANCELLED` — плюс **`PAUSED`** (Suspended) и **`CANCELLING`** требуют обработки.
...
```python
# ── Полный список терминальных и промежуточных state types ───────────────────
_TERMINAL_STATES = {"COMPLETED", "FAILED", "CRASHED", "CANCELLED"}
_WARN_STATES     = {"PAUSED", "CANCELLING"}  # не terminal, но требуют UI-сигнала
...
def _poll_run() -> dict | None:
    ...
    if run.get("run_id") and run["state"] not in _TERMINAL_STATES:
        ...
        if info:
            ...
            # ← state.type для логики, state.name для отображения
            state_type = state_obj.get("type", run["state"])
            state_name = state_obj.get("name", state_type)  # "Retrying", "Late" и т.д.
...
def _fetch_child_runs(parent_run_id: str) -> list[dict]:
    """Запрос дочерних flow runs для подсчёта прогресса."""
    data = prefect_post("/flow_runs/filter", {
        "flow_runs": {
            "parent_flow_run_id": {"any_": [parent_run_id]}
        },
        "limit": 50,
    })
...
И в `render()` заменить блок прогресса:
...
if not run:
    ...
    state      = run["state"]
    state_name = run.get("state_name", state)  # человекочитаемое имя
    is_terminal = state in _TERMINAL_STATES
    color = _STATE_COLOR.get(state, "#8B949E")
    ...
    # ── Реальный прогресс через child flow runs ───────────────────────────
    if run.get("run_id") and not is_terminal:
        ...
        progress_val = completed / _TOTAL_STEPS if _TOTAL_STEPS else 0
    ...
    # ── Специфичные сообщения по state ───────────────────────────────────
...
| Проблема | Было | Стало |
| :-- | :-- | :-- |
| `state.name` игнорировался | `state.type` → `"RUNNING"` | `state.name` → `"Retrying"`, `"Late"` |
| Прогресс фиксированный | `0.05` всегда | Реальный счётчик через `parent_flow_run_id` |
| `_TERMINAL_STATES` неполный | 4 состояния | + `PAUSED`, `CANCELLING` в `_WARN_STATES` |
| `SCHEDULED` = Late не обрабатывался | Нет предупреждения | Отдельное сообщение |

Для работы `_fetch_child_runs` нужно убедиться что `prefect_post` в `ui/common.py` поддерживает endpoint `/flow_runs/filter` — если нет, покажи код `common.py` и добавим.
